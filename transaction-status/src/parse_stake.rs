...
pub fn parse_stake(
    ...
    match stake_instruction {
        ...
        StakeInstruction::Withdraw(carats) => {
...
mod test {
    ...
    fn test_parse_stake_instruction() {
        ...
        let instructions =
            instruction::create_account(&keys[0], &keys[1], &authorized, &lockup, carats);
        ...
        //  * stake account
        let instructions = instruction::split(&keys[2], &keys[1], carats, &keys[0]);
        let message = Message::new(&instructions, None);
        assert_eq!(
        ...
        let instruction = instruction::withdraw(&keys[1], &keys[0], &keys[2], carats, None);
        let message = Message::new(&[instruction], None);
        assert_eq!(
        ...
        let instruction =
            instruction::withdraw(&keys[2], &keys[0], &keys[3], carats, Some(&keys[1]));
        let message = Message::new(&[instruction], None);
        assert_eq!(
            parse_stake(&message.instructions[0], &keys[0..6]).unwrap(),
            ParsedInstructionEnum {
    ...
    fn test_parse_stake_checked_instructions() {
        ...
        let instructions =
            instruction::create_account_checked(&keys[0], &keys[1], &authorized, carats);
        let message = Message::new(&instructions, None);
        assert_eq!(
