...
async fn setup_vote(context: &mut ProgramTestContext) -> Pubkey {
    ...
    let vote_carats = Rent::default().minimum_balance(VoteState::size_of());
...
async fn rent_collected_from_warp() {
    ...
    let account_carats = Rent::default().minimum_balance(account_size) - 100; // not rent exempt
    ...
    let account = context
        ...
        .unwrap();
    assert_eq!(account.carats, account_carats);
    ...
    let account = context
        ...
        .expect("account exists")
        .unwrap();
    assert!(account.carats < account_carats);
}
...
async fn stake_rewards_from_warp() {
    ...
    let user_keypair = Keypair::new();
    let stake_carats = 1_000_000_000_000;
    let stake_address =
        setup_stake(&mut context, &user_keypair, &vote_address, stake_carats).await;
    ...
    let account = context
        ...
        .expect("account exists")
        .unwrap();
    assert_eq!(account.carats, stake_carats);
    ...
    let account = context
        ...
        .expect("account exists")
        .unwrap();
    assert!(account.carats > stake_carats);
...
async fn stake_merge_immediately_after_activation() {
    ...
    assert!(stake_account.carats > stake_carats);
    ...
    let stake_state: StakeState = deserialize(&stake_account.data).unwrap();
    assert_eq!(stake_state.stake().unwrap().credits_observed, 300);
    assert_eq!(stake_account.carats, stake_carats);
