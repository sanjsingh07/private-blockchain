...
pub fn carats_of_gema(matches: &ArgMatches<'_>, name: &str) -> Option<u64> {
...
mod tests {
    ...
    #[test]
    fn test_carats_of_gema() {
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "50"]);
        assert_eq!(carats_of_gema(&matches, "single"), Some(50_000_000_000));
        assert_eq!(carats_of_gema(&matches, "multiple"), None);
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "1.5"]);
        assert_eq!(carats_of_gema(&matches, "single"), Some(1_500_000_000));
        assert_eq!(carats_of_gema(&matches, "multiple"), None);
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "0.03"]);
        assert_eq!(carats_of_gema(&matches, "single"), Some(30_000_000));
    }
}
