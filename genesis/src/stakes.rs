...
// carats required to run staking operations for one year
//  the staker account needs carry enough
//  carats to cover TX fees (delegation) for one year,
//  and we support one delegation per epoch
fn calculate_staker_fees(genesis_config: &GenesisConfig, years: f64) -> u64 {
    genesis_config.fee_rate_governor.max_carats_per_signature
...
/// create stake accounts for carats with at most stake_granularity in each
///  account
pub fn create_and_add_stakes(
    ...
    // the largest each stake account should be, in carats
    ...
    let total_carats = staker_info.carats;
    ...
    let mut stakes_carats = total_carats - staker_fees;
    ...
    genesis_config
        ...
        .or_insert_with(|| {
            stakes_carats -= staker_rent_reserve;
    ...
    for unlock in unlocks {
        let carats = unlock.amount(stakes_carats);

        let (granularity, remainder) = if granularity < carats {
            (granularity, carats % granularity)
        } else {
            (carats, 0)
        };
        ...
        for _ in 0..(carats / granularity).saturating_sub(1) {
...
mod tests {
    ...
    fn create_and_check_stakes(
        ...
        assert!(genesis_config
            .accounts
            .iter()
            .all(|(_pubkey, account)| account.carats <= granularity
                || account.carats - granularity
    ...
    fn test_create_stakes() {
        ...
        let total_carats = staker_reserve + reserve * 2 + 1;
        ...
        let total_carats = staker_reserve + (granularity + reserve) * 2;
        create_and_check_stakes(
            &mut GenesisConfig {
                rent,
                ..GenesisConfig::default()
        ...
        let granularity = reserve * 3;
        let total_carats = staker_reserve + (granularity + reserve + 1) * 2;
        create_and_check_stakes(
            &mut GenesisConfig {
                rent,
                ..GenesisConfig::default()
