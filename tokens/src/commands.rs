...
use solana_sdk::{
    ...
    message::Message,
    native_token::{carats_to_gema, gema_to_carats},
...
fn transfer<S: Signer>(
    ...
    let create_instruction =
        system_instruction::transfer(&sender_keypair.pubkey(), to_pubkey, carats);
...
fn distribution_instructions(
    ...
    match &args.stake_args {
        ...
        None => {
            ...
            let carats = allocation.amount;
            let instruction = system_instruction::transfer(&from, &to, carats);
            vec![instruction]
        }
    ...
    for allocation in allocations.iter() {
        ...
        let do_create_associated_token_account = if let Some(spl_token_args) = &args.spl_token_args
        {
            ...
            println!(
                "{:<44}  {:>24.9}",
                allocation.recipient,
                carats_to_gema(allocation.amount)
            );
            false
        };
...
fn check_payer_balances(
    ...
    if let Some((unlocked_gema_source, total_unlocked_gema)) = unlocked_gema_source {
        ...
        if fee_payer_balance < fees + undistributed_tokens {
            return Err(Error::InsufficientFunds(
                vec![FundingSource::SystemAccount, FundingSource::FeePayer].into(),
                carats_to_gema(fees + undistributed_tokens).to_string(),
            ));
        }
    } else {
        if fee_payer_balance < fees {
...
pub fn process_balances(client: &RpcClient, args: &BalancesArgs) -> Result<(), Error> {
    ...
    for allocation in &allocations {
        if let Some(spl_token_args) = &args.spl_token_args {
            ...
            let expected = carats_to_gema(allocation.amount);
            let actual = carats_to_gema(client.get_balance(&address).unwrap());
            println!(
                "{:<44}  {:>24.9}  {:>24.9}  {:>24.9}",
                allocation.recipient,
                expected,
...
pub fn test_process_distribute_tokens_with_client(
    ...
    wtr.write_record(&[
        ...
        carats_to_gema(expected_amount).to_string(),
    ])
    .unwrap();
    wtr.flush().unwrap();
...
pub fn test_process_create_stake_with_client(client: &RpcClient, sender_keypair: Keypair) {
    ...
    let expected_amount = gema_to_carats(1000.0);
    let alice_pubkey = solana_sdk::pubkey::new_rand();
    ...
    assert_eq!(
        ...
        expected_amount - gema_to_carats(1.0),
    );
...
mod tests {
    ...
    fn test_process_transfer_amount_allocations() {
        ...
        let client = RpcClient::new_with_commitment(url, CommitmentConfig::processed());
        test_process_distribute_tokens_with_client(&client, alice, Some(gema_to_carats(1.5)));
    }
    ...
    fn test_check_payer_balances_distribute_stakes_single_payer() {
        ...
        let expensive_allocations = vec![Allocation {
            ...
            amount: gema_to_carats(expensive_allocation_amount),
            lockup_date: "".to_string(),
        }];
