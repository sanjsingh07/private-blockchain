...
pub enum SystemInstruction {
    ...
    CreateAccount {
        /// Number of carats to transfer to the new account
        carats: u64,
    ...
    ///
    /// The `u64` parameter is the carats to withdraw, which must leave the
    ...
    /// Transfer carats from a derived address
    ///
    /// # Account references
...
pub fn transfer(from_pubkey: &Pubkey, to_pubkey: &Pubkey, carats: u64) -> Instruction {
    let account_metas = vec![
        AccountMeta::new(*from_pubkey, true),
        AccountMeta::new(*to_pubkey, false),
    ];
...
pub fn transfer_many(from_pubkey: &Pubkey, to_carats: &[(Pubkey, u64)]) -> Vec<Instruction> {
    to_carats
        .iter()
        .map(|(to_pubkey, carats)| transfer(from_pubkey, to_pubkey, *carats))
        .collect()
}
...
mod tests {
    ...
    fn test_move_many() {
        ...
        let to_carats = vec![(bob_pubkey, 1), (carol_pubkey, 2)];

        let instructions = transfer_many(&alice_pubkey, &to_carats);
