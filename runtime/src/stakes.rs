...
impl Stakes {
    ...
    pub fn vote_balance_and_staked(&self) -> u64 {
        self.stake_delegations
            ...
            + self
                .vote_accounts
                .iter()
                .map(|(_pubkey, (_staked, vote_account))| vote_account.carats())
                .sum::<u64>()
    }
    ...
    pub fn store(
        ...
        if solana_vote_program::check_id(account.owner()) {
            ...
            // when account is removed (carats == 0 or data uninitialized), don't read so that
            ...
            //  old_stake is stake carats and voter_pubkey from the pre-store() version
            ...
            let stake = delegation.map(|delegation| {
                (
                    ...
                    if account.carats() != 0 {
                        ...
                        // when account is removed (carats == 0), this special `else` clause ensures
            ...
            if remove_delegation {
                // when account is removed (carats == 0), remove it from Stakes as well
            ...
            // Runtime always enforces an intermediary write of account.carats == 0,
    ...
    fn test_vote_balance_and_staked_normal() {
        ...
        impl Stakes {
            pub fn vote_balance_and_warmed_staked(&self) -> u64 {
                self.vote_accounts
                    .iter()
                    .map(|(_pubkey, (staked, account))| staked + account.carats())
                    .sum()
            }
        }
        ...
        for (epoch, expected_warmed_stake) in ((genesis_epoch + 1)..=3).zip(&[2, 3, 4]) {
            ...
            // vote_balance_and_staked() always remain to return same carats
