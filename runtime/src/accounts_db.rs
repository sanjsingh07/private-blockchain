...
pub struct AccountInfo {
    ...
    /// carats in the account used when squashing kept for optimization
...
struct FrozenAccountInfo {
    ...
    pub carats: u64, // Account balance cannot be lower than this amount
}
...
pub struct AccountsDb {
    ...
    /// Accounts that will cause a panic! if data modified or carats decrease
...
impl AccountsDb {
    ...
    fn filter_zero_lamport_clean_for_incremental_snapshots(
        ...
        purges_zero_carats.retain(|pubkey, (slot_account_infos, _ref_count)| {
            // Only keep purges_zero_carats where the entire history of the account in the root set
    ...
    >(
        ...
        let accounts_and_meta_to_store: Vec<_> = accounts
            ...
            .map(|(pubkey, account)| {
                ...
                // None = store a default/empty account with 0 carats
    ...
    fn calculate_accounts_hash(
        ...
        // We'll also accumulate the carats within each chunk and fewer chunks results in less contention to accumulate the sum.
...
pub mod tests {
    ...
    fn test_clean_zero_lamport_and_dead_slot() {
        ...
        // zero carats, and are not present in any other slot's
        // storage entries
    ...
    fn test_accounts_db_serialize1() {
        ...
        // Overwrite account 30 from slot 0 with carats=0 into slot 1.
        ...
        accounts.clean_accounts(None, false, None);
        // Overwrite account 31 from slot 0 with carats=0 into slot 2.
    ...
    fn test_hash_frozen_account_data() {
        ...
        // Carats changes to not affect the hash
    ...
    fn test_frozen_account_lamport_increase() {
        ...
        // Store with an increase in carats is ok
        account.set_carats(2);
        db.store_uncached(0, &[(&frozen_pubkey, &account)]);

        // Store with an decrease that does not go below the frozen amount of carats is tolerated
    ...
    fn test_frozen_account_lamport_decrease() {
        ...
        // Store with a decrease below the frozen amount of carats is not ok
    ...
    fn test_storage_finder() {
        ...
        let carats = 100;
        let data_len = 8190;
        let account = AccountSharedData::new(carats, data_len, &solana_sdk::pubkey::new_rand());
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                loop {
                    ...
                    // slot + 1 == account.carats because of the account-cache-flush thread
                    assert_eq!(
    ...
    fn test_purge_alive_unrooted_slots_after_clean() {
        ...
        // Store accounts with greater than 0 carats
        let account = AccountSharedData::new(1, 1, AccountSharedData::default().owner());
        accounts.store_uncached(slot0, &[(&shared_key, &account)]);
